...
def _job_scope_to_storage_key(job_scope):
    ...
    :param dict job_scope: The job scope (dict representation)
    :return string: The full S3 key to use
    """
    prefix = hashlib.md5(job_scope['ad_account_id'].encode()).hexdigest()[:6]

    actual_report_time = tztools.dt_to_other_timezone(
        job_scope['report_time'].replace(microsecond=0),
        destination_timezone_name='UTC'
    )

    zulu_time = actual_report_time.strftime('%Y-%m-%dT%H:%M:%SZ')

    return f'{job_scope["platform"]}/' \
           f'{prefix}-{job_scope["ad_account_id"]}/' \
           f'{job_scope["report_type"]}/' \
           f'{actual_report_time.strftime("%Y")}/' \
           f'{actual_report_time.strftime("%m")}/' \
           f'{actual_report_time.strftime("%d")}/' \
           f'{zulu_time}-' \
           f'{job_scope["report_id"]}.json'


def store(item, scope):
    """
    ...
    :param dict item: The raw data to store
    :param dict scope: The job scope information
    :return string: The key used to store the data
    """
    key = _job_scope_to_storage_key(scope)
