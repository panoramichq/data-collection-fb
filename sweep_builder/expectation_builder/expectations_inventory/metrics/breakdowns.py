...
def daily_metrics_per_ad_account(entity_type, report_type, reality_claim):
    # type: (str, str, RealityClaim) -> Generator[ExpectationClaim]
    """Make ad-account-level expectation claim from a given entity reality claim."""
    ...
    for day in date_range(range_start, range_end):
        yield ExpectationClaim(
            reality_claim_data,
            job_signatures=[JobSignature.bind(generate_id(
                ad_account_id=reality_claim.ad_account_id,
                range_start=day,
...
# per-ad account generators

hour_metrics_per_campaign_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Campaign,
    ReportType.day_hour,
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

hour_metrics_per_adset_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.AdSet,
    ReportType.day_hour,
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

hour_metrics_per_ad_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Ad,
    ReportType.day_hour,
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

day_metrics_per_ad_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Ad,
    ReportType.day,
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

day_age_gender_metrics_per_ad_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Ad,
    ReportType.day_age_gender,
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

day_dma_metrics_per_ad_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Ad,
    ReportType.day_dma
)  # type: (RealityClaim) -> Generator[ExpectationClaim]

day_platform_metrics_per_ad_per_ad_account = functools.partial(
    daily_metrics_per_ad_account,
    Entity.Ad,
    ReportType.day_platform
)  # type: (RealityClaim) -> Generator[ExpectationClaim]
